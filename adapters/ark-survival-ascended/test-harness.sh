#!/usr/bin/env bash
# ARK: Survival Ascended Adapter Test Harness
set -euo pipefail

ADAPTER_ID="ark-survival-ascended"
TEST_CONTAINER="ark-test-$(date +%s)"
TEST_DATA_DIR="/tmp/ark-test-data"
RESULTS_FILE="/tmp/ark-test-results.json"
PASS=0
FAIL=0
...
log "=== ARK: Survival Ascended Adapter Test Harness ==="
log "Container: $TEST_CONTAINER"

mkdir -p "$TEST_DATA_DIR/SavedArks"
...
docker run -d \
  ...
  -e SERVER_ADMIN_PASSWORD="adminpass" \
  -e MAX_PLAYERS=10 \
  -v "$TEST_DATA_DIR:/data" \
  mschnitzer/asa-linux-server &>/dev/null
...
if docker exec "$TEST_CONTAINER" pgrep -f ArkAscendedServer &>/dev/null; then
  pass "ArkAscendedServer process found"
else
  fail "ArkAscendedServer process not found"
fi

# Test 3: Log output
log "Test 3: Server log output..."
logs=$(docker logs "$TEST_CONTAINER" 2>&1 | head -50)
if echo "$logs" | grep -qi "ark\|starting\|loading"; then
...
if [[ -d "$TEST_DATA_DIR/SavedArks" ]]; then
  pass "Backup path exists: $TEST_DATA_DIR/SavedArks"
else
  fail "Backup path missing: $TEST_DATA_DIR/SavedArks"
fi
...
touch "$TEST_DATA_DIR/SavedArks/TheIsland.ark"
backup_file="/tmp/ark-backup-test.tar.gz"
...
restore_dir="/tmp/ark-restore-test"
mkdir -p "$restore_dir"
tar -xzf "$backup_file" -C "$restore_dir" 2>/dev/null
if [[ -d "$restore_dir/SavedArks" ]]; then
