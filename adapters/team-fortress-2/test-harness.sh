#!/usr/bin/env bash
# Team Fortress 2 Adapter Test Harness
set -euo pipefail

ADAPTER_ID="team-fortress-2"
TEST_CONTAINER="tf2-test-$(date +%s)"
TEST_DATA_DIR="/tmp/tf2-test-data"
RESULTS_FILE="/tmp/tf2-test-results.json"
PASS=0
FAIL=0
...
log "=== Team Fortress 2 Adapter Test Harness ==="
...
docker run -d \
  ...
  -e SRCDS_STARTMAP="cp_dustbowl" \
  -v "$TEST_DATA_DIR/cfg:/opt/tf2/tf/cfg" \
  cm2network/tf2 &>/dev/null
...
if docker exec "$TEST_CONTAINER" pgrep -f srcds &>/dev/null; then
  pass "srcds process found"
else
  fail "srcds process not found"
fi

# Test 3: Log output
log "Test 3: Server log output..."
logs=$(docker logs "$TEST_CONTAINER" 2>&1 | head -50)
if echo "$logs" | grep -qi "tf2\|starting\|loading"; then
...
backup_file="/tmp/tf2-backup-test.tar.gz"
...
restore_dir="/tmp/tf2-restore-test"
mkdir -p "$restore_dir"
