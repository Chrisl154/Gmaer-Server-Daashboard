#!/usr/bin/env bash
# 7 Days to Die Adapter Test Harness
set -euo pipefail

ADAPTER_ID="7-days-to-die"
TEST_CONTAINER="7dtd-test-$(date +%s)"
TEST_DATA_DIR="/tmp/7dtd-test-data"
RESULTS_FILE="/tmp/7dtd-test-results.json"
PASS=0
FAIL=0
...
log "=== 7 Days to Die Adapter Test Harness ==="
...
if docker exec "$TEST_CONTAINER" pgrep -f startserver.sh &>/dev/null; then
  pass "startserver.sh process found"
else
  fail "startserver.sh process not found"
fi

# Test 3: Log output
log "Test 3: Server log output..."
logs=$(docker logs "$TEST_CONTAINER" 2>&1 | head -50)
if echo "$logs" | grep -qi "7dtd\|starting\|loading"; then
...
touch "$TEST_DATA_DIR/Saves/world.bin"
backup_file="/tmp/7dtd-backup-test.tar.gz"
...
restore_dir="/tmp/7dtd-restore-test"
mkdir -p "$restore_dir"
tar -xzf "$backup_file" -C "$restore_dir" 2>/dev/null
if [[ -d "$restore_dir/Saves" ]]; then
