#!/usr/bin/env bash
# Factorio Adapter Test Harness
set -euo pipefail

ADAPTER_ID="factorio"
TEST_CONTAINER="factorio-test-$(date +%s)"
TEST_DATA_DIR="/tmp/factorio-test-data"
RESULTS_FILE="/tmp/factorio-test-results.json"
PASS=0
FAIL=0
...
log "=== Factorio Adapter Test Harness ==="
log "Container: $TEST_CONTAINER"

mkdir -p "$TEST_DATA_DIR/saves"
...
docker run -d \
  ...
  -e PORT=34197 \
  -e RCON_PORT=27015 \
  -e SAVE_NAME="test-world" \
  -e GENERATE_NEW_SAVE=true \
  -v "$TEST_DATA_DIR:/factorio" \
  factoriotools/factorio &>/dev/null
...
if docker exec "$TEST_CONTAINER" pgrep -f factorio &>/dev/null; then
  pass "factorio process found"
else
  fail "factorio process not found"
fi

# Test 3: Log output
log "Test 3: Server log output..."
logs=$(docker logs "$TEST_CONTAINER" 2>&1 | head -50)
if echo "$logs" | grep -qi "factorio\|starting\|loading"; then
...
if [[ -d "$TEST_DATA_DIR/saves" ]]; then
  pass "Backup path exists: $TEST_DATA_DIR/saves"
else
  fail "Backup path missing: $TEST_DATA_DIR/saves"
fi
...
touch "$TEST_DATA_DIR/saves/test-world.zip"
backup_file="/tmp/factorio-backup-test.tar.gz"
...
restore_dir="/tmp/factorio-restore-test"
mkdir -p "$restore_dir"
tar -xzf "$backup_file" -C "$restore_dir" 2>/dev/null
if [[ -f "$restore_dir/saves/test-world.zip" ]]; then
