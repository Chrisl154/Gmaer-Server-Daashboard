#!/usr/bin/env bash
# Project Zomboid Adapter Test Harness
set -euo pipefail

ADAPTER_ID="project-zomboid"
TEST_CONTAINER="pz-test-$(date +%s)"
TEST_DATA_DIR="/tmp/pz-test-data"
RESULTS_FILE="/tmp/pz-test-results.json"
PASS=0
FAIL=0
...
log "=== Project Zomboid Adapter Test Harness ==="
log "Container: $TEST_CONTAINER"

mkdir -p "$TEST_DATA_DIR/Zomboid/Saves"
...
docker run -d \
  ...
  asledgehammer/project-zomboid-server &>/dev/null
...
if docker exec "$TEST_CONTAINER" pgrep -f ProjectZomboid &>/dev/null; then
  pass "ProjectZomboid process found"
else
  fail "ProjectZomboid process not found"
fi

# Test 3: Log output
log "Test 3: Server log output..."
logs=$(docker logs "$TEST_CONTAINER" 2>&1 | head -50)
if echo "$logs" | grep -qi "zomboid\|starting\|loading"; then
...
if [[ -d "$TEST_DATA_DIR/Zomboid/Saves" ]]; then
  pass "Backup path exists: $TEST_DATA_DIR/Zomboid/Saves"
else
  fail "Backup path missing: $TEST_DATA_DIR/Zomboid/Saves"
fi
...
touch "$TEST_DATA_DIR/Zomboid/Saves/world.db"
backup_file="/tmp/pz-backup-test.tar.gz"
...
restore_dir="/tmp/pz-restore-test"
mkdir -p "$restore_dir"
tar -xzf "$backup_file" -C "$restore_dir" 2>/dev/null
if [[ -f "$restore_dir/Zomboid/Saves/world.db" ]]; then
