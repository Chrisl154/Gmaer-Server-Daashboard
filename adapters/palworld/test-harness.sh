#!/usr/bin/env bash
# Palworld Adapter Test Harness
# Tests: start, stop, console attach, backup, restore in ephemeral container
set -euo pipefail

ADAPTER_ID="palworld"
TEST_CONTAINER="palworld-test-$(date +%s)"
TEST_DATA_DIR="/tmp/palworld-test-data"
RESULTS_FILE="/tmp/palworld-test-results.json"
PASS=0
FAIL=0
...
log "=== Palworld Adapter Test Harness ==="
...
docker run -d \
  ...
  -e COMMUNITY=false \
  -v "$TEST_DATA_DIR:/palworld/Pal/Saved" \
  thijsvanloef/palworld-server-docker:latest &>/dev/null
...
if docker exec "$TEST_CONTAINER" pgrep -f PalServer-Linux &>/dev/null; then
  pass "PalServer-Linux process found"
else
  fail "PalServer-Linux process not found"
fi

# Test 3: Log output
log "Test 3: Server log output..."
logs=$(docker logs "$TEST_CONTAINER" 2>&1 | head -50)
if echo "$logs" | grep -qi "palworld\|starting\|loading"; then
...
touch "$TEST_DATA_DIR/Level.sav"
mkdir -p "$TEST_DATA_DIR/Players"
backup_file="/tmp/palworld-backup-test.tar.gz"
...
restore_dir="/tmp/palworld-restore-test"
mkdir -p "$restore_dir"
tar -xzf "$backup_file" -C "$restore_dir" 2>/dev/null
if [[ -f "$restore_dir/Level.sav" ]]; then
