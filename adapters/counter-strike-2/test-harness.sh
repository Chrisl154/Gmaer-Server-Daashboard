#!/usr/bin/env bash
# Counter-Strike 2 Adapter Test Harness
set -euo pipefail

ADAPTER_ID="counter-strike-2"
TEST_CONTAINER="cs2-test-$(date +%s)"
TEST_DATA_DIR="/tmp/cs2-test-data"
RESULTS_FILE="/tmp/cs2-test-results.json"
PASS=0
FAIL=0
...
log "=== Counter-Strike 2 Adapter Test Harness ==="
...
docker run -d \
  --name "$TEST_CONTAINER" \
  --memory=4g \
  -e CS2_SERVERNAME="TestServer" \
  -e CS2_PORT=27015 \
  -e CS2_MAXPLAYERS=10 \
  -e RCON_PASSWORD="testpass" \
  -v "$TEST_DATA_DIR:/opt/cs2/game/csgo/cfg" \
  joedwards32/cs2 &>/dev/null
...
if docker exec "$TEST_CONTAINER" pgrep -f "linuxsteamrt64/cs2" &>/dev/null; then
  pass "cs2 process found"
else
  fail "cs2 process not found"
fi

# Test 3: Log output
log "Test 3: Server log output..."
logs=$(docker logs "$TEST_CONTAINER" 2>&1 | head -50)
if echo "$logs" | grep -qi "cs2\|starting\|loading"; then
...
touch "$TEST_DATA_DIR/server.cfg"
backup_file="/tmp/cs2-backup-test.tar.gz"
...
restore_dir="/tmp/cs2-restore-test"
mkdir -p "$restore_dir"
tar -xzf "$backup_file" -C "$restore_dir" 2>/dev/null
if [[ -f "$restore_dir/server.cfg" ]]; then
