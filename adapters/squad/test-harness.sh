#!/usr/bin/env bash
# Squad Adapter Test Harness
set -euo pipefail

ADAPTER_ID="squad"
TEST_CONTAINER="squad-test-$(date +%s)"
TEST_DATA_DIR="/tmp/squad-test-data"
RESULTS_FILE="/tmp/squad-test-results.json"
PASS=0
FAIL=0
...
log "=== Squad Adapter Test Harness ==="
log "Container: $TEST_CONTAINER"

mkdir -p "$TEST_DATA_DIR/ServerConfig"
...
docker run -d \
  ...
  -v "$TEST_DATA_DIR:/opt/squad/SquadGame/ServerConfig" \
  andrewmhub/squad-server &>/dev/null
...
if docker exec "$TEST_CONTAINER" pgrep -f SquadGameServer &>/dev/null; then
  pass "SquadGameServer process found"
else
  fail "SquadGameServer process not found"
fi

# Test 3: Log output
log "Test 3: Server log output..."
logs=$(docker logs "$TEST_CONTAINER" 2>&1 | head -50)
if echo "$logs" | grep -qi "squad\|starting\|loading"; then
...
touch "$TEST_DATA_DIR/ServerConfig/Server.cfg"
backup_file="/tmp/squad-backup-test.tar.gz"
...
restore_dir="/tmp/squad-restore-test"
mkdir -p "$restore_dir"
tar -xzf "$backup_file" -C "$restore_dir" 2>/dev/null
if [[ -f "$restore_dir/ServerConfig/Server.cfg" ]]; then
