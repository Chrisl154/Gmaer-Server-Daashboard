#!/usr/bin/env bash
# Enshrouded Adapter Test Harness
# Tests: start, stop, console attach, backup, restore in ephemeral container
set -euo pipefail

ADAPTER_ID="enshrouded"
TEST_CONTAINER="enshrouded-test-$(date +%s)"
TEST_DATA_DIR="/tmp/enshrouded-test-data"
RESULTS_FILE="/tmp/enshrouded-test-results.json"
PASS=0
FAIL=0
...
log "=== Enshrouded Adapter Test Harness ==="
...
docker run -d \
  ...
  -v "$TEST_DATA_DIR:/home/enshrouded/steamapps/common/EnshroudedServer/savegame" \
  sknnr/enshrouded-server:latest &>/dev/null
...
if docker exec "$TEST_CONTAINER" pgrep -f enshrouded_server &>/dev/null; then
  pass "enshrouded_server process found"
else
  fail "enshrouded_server process not found"
fi

# Test 3: Log output
log "Test 3: Server log output..."
logs=$(docker logs "$TEST_CONTAINER" 2>&1 | head -50)
if echo "$logs" | grep -qi "enshrouded\|starting\|loading"; then
...
touch "$TEST_DATA_DIR/savegame.db"
backup_file="/tmp/enshrouded-backup-test.tar.gz"
...
restore_dir="/tmp/enshrouded-restore-test"
mkdir -p "$restore_dir"
tar -xzf "$backup_file" -C "$restore_dir" 2>/dev/null
if [[ -f "$restore_dir/savegame.db" ]]; then
