#!/usr/bin/env bash
# Don't Starve Together Adapter Test Harness
set -euo pipefail

ADAPTER_ID="dont-starve-together"
TEST_CONTAINER="dst-test-$(date +%s)"
TEST_DATA_DIR="/tmp/dst-test-data"
RESULTS_FILE="/tmp/dst-test-results.json"
PASS=0
FAIL=0
...
log "=== Don't Starve Together Adapter Test Harness ==="
log "Container: $TEST_CONTAINER"

mkdir -p "$TEST_DATA_DIR/clusters/MyDSTServer"
...
docker run -d \
  --name "$TEST_CONTAINER" \
  --memory=2g \
  -e CLUSTER_NAME="MyDSTServer" \
  -e CLUSTER_TOKEN="test-token" \
  -e GAME_MODE="survival" \
  -e MAX_PLAYERS=6 \
  -v "$TEST_DATA_DIR:/data" \
  jamesits/dst-dedicated-server &>/dev/null
...
if docker exec "$TEST_CONTAINER" pgrep -f dontstarve_dedicated &>/dev/null; then
  pass "dontstarve_dedicated process found"
else
  fail "dontstarve_dedicated process not found"
fi

# Test 3: Log output
log "Test 3: Server log output..."
logs=$(docker logs "$TEST_CONTAINER" 2>&1 | head -50)
if echo "$logs" | grep -qi "dst\|starting\|loading"; then
...
if [[ -d "$TEST_DATA_DIR/clusters" ]]; then
  pass "Backup path exists: $TEST_DATA_DIR/clusters"
else
  fail "Backup path missing: $TEST_DATA_DIR/clusters"
fi
...
touch "$TEST_DATA_DIR/clusters/MyDSTServer/cluster.ini"
backup_file="/tmp/dst-backup-test.tar.gz"
...
restore_dir="/tmp/dst-restore-test"
mkdir -p "$restore_dir"
tar -xzf "$backup_file" -C "$restore_dir" 2>/dev/null
if [[ -d "$restore_dir/clusters" ]]; then
